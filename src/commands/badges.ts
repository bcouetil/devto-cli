...
export async function badges(files?: string[], options?: Partial<BadgesOptions>) {
  ...
  try {
    ...
    for (const [categoryKey, articles] of sortedCategories) {
      if (options.jpg) {
        categorySections.push(renderCategorySvg(categoryKey, articles, badgeUrls, repository, branch));
      } else {
        categorySections.push(renderCategoryHtml(categoryKey, articles));
      }
    ...
    categorySections.push(`_Generated by [a fork of devto-cli](https://github.com/bcouetil/devto-cli) on ${today}_`);
    categorySections.push('');

    const badgesBlock = categorySections.join('\n');
    const existingContent = await fs.pathExists(outputFile) ? await fs.readFile(outputFile, 'utf8') : null;
    const hasMarkers = existingContent?.includes(BADGES_START) && existingContent?.includes(BADGES_END);

    let markdown: string;
    if (hasMarkers) {
      markdown = injectBadgesSection(existingContent!, badgesBlock);
    } else {
      const sections: string[] = ['# 📰 Articles', ''];
      sections.push('Here are my articles, grouped by theme and sorted by publication date (newest first), with view counts and highlights for the most read ones ⭐');
      sections.push('');
      sections.push(BADGES_START);
      sections.push('');
      sections.push(badgesBlock);
      sections.push(BADGES_END);
